...
/** @defgroup STM32_EVAL_SDIO_SD_User_Definition
  * @{
  */
#define SDIO_INIT_CLK_DIV       178    //400KHz
#define SDIO_TRANSFER_CLK_DIV   1      //24MHz
#define SD_DETECT_GPIO_PORT GPIOC
#define SD_DETECT_PIN GPIO_Pin_10


//Pin assignment  
//- SDIO_CK->PC12, SDIO_CMD->PD2, SDIO_D0->PC8, SDIO_D1->PC9, SDIO_D2->PC10, SDIO_D3->PC11
void SD_LowLevel_Init(void)
{
	...
	/*Configure SDIO Pin as alternative function push-pull mode except SDIO_CMD*/
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	/*Configure SDIO_CMD as alternative function push-pull mode*/
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
	GPIO_Init(GPIOD, &GPIO_InitStructure);
}

void SD_LowLevel_DMA_RxConfig(uint32_t *readbuff,uint16_t NumOfByte)
{
	...
	DMA_ClearFlag(DMA2_FLAG_GL4);  //clear DMA2 Channel4 global flag
	
	DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;
	DMA_InitStructure.DMA_Priority = DMA_Priority_High;
	DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralSRC;
	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;
	DMA_InitStructure.DMA_BufferSize = NumOfByte/4;  //4 bytes per SDIO_FIFO word
	...
	DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)(&SDIO->FIFO);  //SDIO->FIFO BaseAddress
	DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Word;
	DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
	DMA_Init(DMA2_Channel4, &DMA_InitStructure);
	
	DMA_Cmd(DMA2_Channel4, ENABLE); //enable DMA Channel
}
...
void SD_LowLevel_DMA_TxConfig(uint32_t *writebuff, uint32_t NumOfByte)
{
	DMA_InitTypeDef DMA_InitStructure;
	...
	DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;
	DMA_InitStructure.DMA_BufferSize = NumOfByte/4;  //4 bytes per SDIO_FIFO word
	DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)writebuff;
...
SD_Error SD_Init(void)
{
  ...
  SDIO_DeInit();  //reset the SDIO registers
...
SD_Error SD_PowerON(void)
{
  ...
  if (errorstatus == SD_OK)
  {
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
    {
      ...
      response = SDIO_GetResponse(SDIO_RESP1);  //get OCR register
      validvoltage = (((response >> 31) == 1) ? 1 : 0);  //check OCR[31] busy bit
      count++;
    }
    if (count >= SD_MAX_VOLT_TRIAL)
    {
      ...
      return(errorstatus);
		...
		//Check OCR[30] CCS bit.If CCS equals to 1,the card is 2.0 high capacity card,
		//else the card is 2.0 standard capacity card
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  else if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    ...
    cardinfo->CardCapacity = (cardinfo->SD_csd.DeviceSize + 1) * 512;  //Unit: KB
...
/**
  ...
  * @param  WriteAddr: Address from where data are to be written.   
  * @param  BlockSize: the SD card Data block size. The Block size should be 512.
  * @retval SD_Error: SD Card Error code.
  */
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_SD_APP_SET_WR_BLK_ERASE_COUNT;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&SDIO_CmdInitStructure);

  errorstatus = CmdResp1Error(SD_CMD_SD_APP_SET_WR_BLK_ERASE_COUNT);
  if (errorstatus != SD_OK)
  {
    return(errorstatus);
  }
...
SD_Error SD_ProcessIRQSrc(void)
{
  ...
  {
    SDIO->ARG = 0x0;
    SDIO->CMD = 0x44C;  //0100_0100_1100B: ATACMD, ShrotResponse, CMD12(stop transfer)
