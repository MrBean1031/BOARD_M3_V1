...
/* 挂载SPI读写函数 */
#define RC522_SPI_ReceiveByte()       RC522_SPI_Transmit(macDummy_Data)
#define RC522_SPI_SendByte(value)     RC522_SPI_Transmit((uint8_t)(value))
 ...
 * 调用  ：内部调用
 */
static u8 ReadRawRC ( u8 ucAddress )
{
	u8 ucAddr, ucReturn;
	...
	ucReturn = RC522_SPI_ReceiveByte ();
	macRC522_CS_Disable();
	return ucReturn;
}
 ...
 * 调用  ：内部调用
 */
static void WriteRawRC ( u8 ucAddress, u8 ucValue )
{  
	u8 ucAddr;
...
void M500PcdConfigISOType ( u8 ucType )
{
	if ( ucType == 'A')                     //ISO14443_A
  {
    ...
    WriteRawRC ( ModeReg, 0x3D );//3F
		...
		WriteRawRC ( TReloadRegL, 50 ); 
		WriteRawRC ( TReloadRegH, 0 );
    //Tprescler=3389, tP=500us
...
static char PcdComMF522 ( u8 ucCommand, u8 * pInData, u8 ucInLenByte, u8 * pOutData, u32 * pOutLenBit )		
{
	...
	ul = 0xFFFFFFFF;  //根据时钟频率调整，操作M1卡最大等待时间25ms
	do 														//认证与寻卡等待时间	
	{
...
char PcdRequest ( u8 ucReq_code, u8 * pTagType )
{
	...
	WriteRawRC ( BitFramingReg, 0x07 );	  //送的最后一个字节的七位
...
char PcdAnticoll ( u8 * pSnr )
{
	...
	ucComMF522Buf [ 0 ] = PICC_ANTICOLL1;	//SEL 卡片防冲突命令
 ...
 * 调用  ：内部调用
 */
static void CalulateCRC ( u8 * pIndata, u8 ucLen, u8 * pOutData )
{
	u8 uc, ucN;
